...
import { HabboInspectionRow, HabboWiredCreatorToolsView, HabboWiredTab } from './HabboWiredCreatorToolsView';
...
const TRANSACTION_LOGS_PAGE_SIZE = 25;
// Habbo stores these as `1 << checkbox.id`: Everyone is id 0, then
// rights/group members/group admins are ids 1/2/3.
...
{
    return permissions & (WIRED_PERMISSION_RIGHTS | WIRED_PERMISSION_GROUP_MEMBERS | WIRED_PERMISSION_GROUP_ADMINS);
};

const normalizeWiredInspectPermissions = (permissions: number) =>
{
    return permissions & (WIRED_PERMISSION_EVERYONE | WIRED_PERMISSION_RIGHTS | WIRED_PERMISSION_GROUP_MEMBERS | WIRED_PERMISSION_GROUP_ADMINS);
};
...
export const WiredCreatorToolsView: FC<{}> = () =>
{
    ...
    useEffect(() =>
    {
        ...
        const refreshTimer = setInterval(() => requestLogs(false), 2500);
    ...
    const selectedVariable = useMemo(() =>
    {
        ...
        const variable = definitions.find(variable => variable.key === selectedKey) ?? definitions[0];

        return variable
            ? { ...variable, textValues: getVariableTextValues(variable) }
            : variable;
    ...
    const chestLogPageCount = Math.max(1, Math.ceil(filteredChestLogs.length / TRANSACTION_LOGS_PAGE_SIZE));
    ...
    const currentRoomStats = roomStatsRef.current ?? roomStats;
    const inspectionSourceValues = inspectionValues?.values ?? {};
    const availableInspectionVariables = getInspectionVariables(
        inspectionType,
        inspectionValues,
        inspectionType === 'global' ? (currentRoomStats?.globalValues ?? {}) : inspectionSourceValues,
        !!inspectionValues,
        currentRoomStats?.globalVariables ?? []);
    const habboInspectionRows: HabboInspectionRow[] = availableInspectionVariables.map(variable => ({
        key: variable.key,
        label: variable.parentKey ? `${ variable.parentKey } / ${ variable.key }` : variable.key,
        value: getInspectionValue(variable, inspectionType, currentRoomStats?.globalValues ?? {}, inspectionSourceValues),
        rawValue: getRawInspectionValue(variable.key, inspectionType, currentRoomStats?.globalValues ?? {}, inspectionSourceValues),
        editable: canEditInspectionVariable(
            inspectionType,
            variable.key,
            inspectionSourceValues,
            !!inspectionValues,
            currentRoomStats?.globalValues ?? {},
            inspectionType === 'global' ? (currentRoomStats?.globalVariables ?? []) : (inspectionValues?.variables ?? []))
    }));
    const canRemoveSelectedInspectionVariable = inspectionType !== 'global'
        && !!inspectionValues
        && isCreatedInspectionVariable(selectedInspectionVariable)
        && Object.prototype.hasOwnProperty.call(inspectionSourceValues, selectedInspectionVariable);

    return (
        <HabboWiredCreatorToolsView
            visible={ isVisible }
            placementHidden={ isPlacementHidden }
            activeTab={ activeTab as HabboWiredTab }
            ...
            roomStats={ currentRoomStats }
            monitorStats={ currentRoomStats ? getMonitorStats(currentRoomStats) : MONITOR_STATS }
            monitorLogs={ monitorLogs.map(log => ({ ...log, description: MONITOR_LOG_INFO[log.type]?.info || log.message })) }
            hasLogs={ logRows.length > 0 }
            onClearLogs={ () => requestLogs(true) }
            onOpenLogs={ openLogs }
            variableType={ variableType }
            onVariableTypeChange={ setVariableType }
            variables={ getVariableDefinitions(variableType, currentRoomStats) }
            selectedVariable={ selectedVariable }
            onSelectVariable={ selectVariable }
            variableHighlight={ variableHighlight }
            onHighlightVariable={ requestVariableHighlight }
            onClearVariableHighlight={ clearVariableHighlight }
            inspectionType={ inspectionType }
            onInspectionTypeChange={ changeInspectionType }
            keepInspectionSelected={ keepInspectionSelected }
            onKeepInspectionSelectedChange={ setKeepInspectionSelected }
            inspectionPreview={ inspectionPreview }
            inspectionValues={ inspectionValues }
            inspectionRows={ habboInspectionRows }
            pulseKeys={ pulseKeys }
            selectedInspectionVariable={ selectedInspectionVariable }
            onSelectInspectionVariable={ setSelectedInspectionVariable }
            onSetInspectionVariable={ setInspectionVariableValue }
            canRemoveInspectionVariable={ canRemoveSelectedInspectionVariable }
            onRemoveInspectionVariable={ removeInspectionVariable }
            giveVariableOpen={ giveVariableOpen }
            onGiveVariableOpenChange={ setGiveVariableOpen }
            giveVariableName={ giveVariableName }
            onGiveVariableNameChange={ setGiveVariableName }
            giveVariableValue={ giveVariableValue }
            onGiveVariableValueChange={ setGiveVariableValue }
            onGiveInspectionVariable={ giveInspectionVariable }
            chests={ chestLogs }
            onOpenChestLogs={ openChestLogs }
            onRefreshChestLogs={ requestChestLogs }
            onSelectChestLog={ setSelectedChestLog }
            ...
            modifyPermissions={ wiredModifyPermissions }
            onModifyPermissionsChange={ updateWiredModifyPermissions }
            inspectPermissions={ wiredInspectPermissions }
            onInspectPermissionsChange={ updateWiredInspectPermissions }
            showToolbar={ showToolbar }
            onShowToolbarChange={ updateShowToolbar }
            showInspectButton={ showInspectButton }
            onShowInspectButtonChange={ updateShowInspectButton }
            playtestingMode={ playtestingMode }
            onPlaytestingModeChange={ updatePlaytestingMode }
            logsOpen={ logsOpen }
            ...
            logsState={ {
                ...
                setSource: setLogSourceFilter,
                level: logCategoryFilter,
                setLevel: setLogCategoryFilter,
            ...
            chestLogsOpen={ chestLogsOpen }
            onCloseChestLogs={ () => setChestLogsOpen(false) }
            filteredChestLogs={ filteredChestLogs }
            transactionLogsState={ {
                filter: chestLogFilter,
                setFilter: setChestLogFilter,
                type: chestLogTypeFilter,
                setType: setChestLogTypeFilter,
                autoRefresh: autoRefreshChestLogs,
                setAutoRefresh: setAutoRefreshChestLogs,
                page: clampedChestLogPage,
                pageCount: chestLogPageCount,
                setPage: setChestLogPage
            } }
            selectedChestLog={ selectedChestLog }
            onCloseChestLogDetails={ () => setSelectedChestLog(null) }
            variableHighlightPulse={ variableHighlightPulse } />
    );
}
